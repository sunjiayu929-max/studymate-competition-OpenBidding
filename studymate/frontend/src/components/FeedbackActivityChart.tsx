...
export function FeedbackActivityChart({ data }: { data: { name: string; count: number }[] }) {
  if (data.length === 0) {
    return (
      <div className="grid h-60 place-items-center rounded-2xl border border-dashed border-[#D7D1C4] bg-[#FBF8F0] px-5 text-center">
  ...
  return (
    <div className="h-60 w-full" aria-label="近24小时学习行为分布图">
      <ResponsiveContainer minWidth={0} initialDimension={{ width: 320, height: 240 }}>
        <BarChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 4 }}>
          ...
          <YAxis tick={{ fontSize: 11, fill: "#66717B" }} allowDecimals={false} axisLine={false} tickLine={false} />
          ...
          <Bar dataKey="count" name="行为次数" fill="url(#feedbackBarFill)" radius={[8, 8, 2, 2]} maxBarSize={96} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
