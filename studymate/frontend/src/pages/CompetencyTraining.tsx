    ...
    <main className="app-page paper-theme competency-studio min-h-dvh pb-12">
      <div className="w-full px-2 py-3 sm:px-4 sm:py-4 lg:px-5">
        ...
        <div className="competency-longform">
          <motion.section className="competency-section border-b border-[#D8E0E8] px-1 py-10 sm:px-3" initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .08 }}><AgentCollaborationFlow workspace={workspace} /></motion.section>
...
function SectionTitle({ icon: Icon, imageSrc, orbitAccent = false, eyebrow, title, description }: { icon: typeof Target; imageSrc?: string; orbitAccent?: boolean; eyebrow: string; title?: string; description?: string }) {
  return <div className={cn("competency-section-title flex items-start gap-4", orbitAccent && "has-orbit", !title && !description && "is-label-only")}><span className={cn("grid size-11 shrink-0 place-items-center rounded-[14px]", imageSrc && "is-real-icon")}>{imageSrc ? <img src={imageSrc} alt="" aria-hidden="true" /> : <Icon className="size-5" />}</span><div><span className="text-[12px] font-extrabold tracking-[.14em]">{eyebrow}</span>{title && <h2 className="mt-1 text-[24px] font-black tracking-[-.035em] text-[#172E49]">{title}</h2>}{description && <p className="mt-1 text-[14px] font-medium leading-6 text-[#62748A]">{description}</p>}</div>{orbitAccent && <i className="competency-section-orbit" aria-hidden="true"><span className="is-orbit-upper"><img src="/images/section-satellite-v1.png" alt="" /></span><span className="is-orbit-lower"><img src="/images/section-satellite-v1.png" alt="" /></span><b className="is-data-one" /><b className="is-data-two" /><b className="is-data-three" /></i>}</div>
}
