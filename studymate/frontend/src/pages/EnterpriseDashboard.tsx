...
function enterpriseRoleLabel(roleName: string) {
  const normalized = roleName.replaceAll(" ", "").toLowerCase()
  if (normalized === "aiinfra工程师") return "大模型应用开发工程师"
  if (normalized === "aiagent工程师" || normalized === "aiagent开发工程师") return "企业RAG应用实施工程师"
  return roleName
}
        ...
        <div className="mt-5 grid flex-1 content-start gap-2.5 sm:grid-cols-2">
          {data.roles.length ? data.roles.map((role, index) => index === 0 ? (
            <button type="button" key={role.name} onClick={() => setSelectedRole(role.name)} className={cn("rounded-xl border p-4 text-left transition sm:col-span-2", selectedRole === role.name ? "border-[#6D9168] bg-[#EAF4E7] shadow-[0_7px_16px_rgba(82,112,77,.1)]" : "border-[#D8E5D5] bg-[#F3F9F1] hover:border-[#9BB398]")}>
              <div className="flex items-start justify-between gap-3"><div><span className="text-[9px] font-extrabold text-[#6D9168]">核心交付岗位</span><h3 className="mt-1 text-sm font-bold leading-5 text-[#334934]">{enterpriseRoleLabel(role.name)}</h3></div><span className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-black text-[#52704D]">{role.member_count} 人</span></div>
            ...
            <button type="button" key={role.name} onClick={() => setSelectedRole(role.name)} className={cn("rounded-xl border p-3 text-left transition", selectedRole === role.name ? "border-[#6D9168] bg-[#EAF4E7] shadow-[0_7px_16px_rgba(82,112,77,.1)]" : "border-[#E3EAE1] bg-[#FBFDFB] hover:border-[#9BB398]")}>
              <div className="flex items-start justify-between gap-2"><h3 className="text-xs font-bold leading-5 text-[#334934]">{enterpriseRoleLabel(role.name)}</h3><strong className="shrink-0 text-sm text-[#52704D]">{role.member_count} 人</strong></div>
