...
export function EnterpriseHub() {
  ...
  const isAdmin = user?.role === "enterprise_admin" || user?.role === "admin"
  const [context, setContext] = useState<Context | null>(null)
  const [tasks, setTasks] = useState<Task[]>([])
  const [knowledgeBases, setKnowledgeBases] = useState<KnowledgeBase[]>([])
  const [tab, setTab] = useState<"tasks" | "knowledge" | "members">("tasks")
  ...
  const updateContext = async (patch: Partial<Context>) => {
    setBusy("context")
    try {
      const next = await apiPatch<Context>("/learner/context", {
        learner_type: patch.learner_type ?? context?.learner_type ?? "student",
        study_stage: patch.study_stage ?? context?.study_stage ?? "",
        company: patch.company ?? context?.company ?? "",
        target_role: patch.target_role ?? context?.target_role ?? "",
      })
      setContext(next)
    } catch (err) { setError(err instanceof Error ? err.message : "资料保存失败") } finally { setBusy("") }
  }
      ...
      <div className="mx-auto max-w-[1440px] px-3 py-3 sm:px-5 sm:py-5 lg:px-7">
        ...
        <section className="mt-4 overflow-hidden rounded-[26px] border border-[#D6E2D4] bg-[#F4F9F2] p-5 shadow-[0_14px_38px_rgba(59,92,58,.08)] sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            ...
            <div className="grid min-w-[240px] grid-cols-2 gap-2"><InfoChip icon={ClipboardList} label="当前任务" value={`${isAdmin ? tasks.length : learnerTasks.length}`} /><InfoChip icon={Library} label="岗位资料库" value={isAdmin ? `${knowledgeBases.length} 个` : context?.enterprise ? "已授权" : "待加入"} /></div>
          </div>
        </section>
        ...
        {loading ? <div className="grid min-h-[40vh] place-items-center"><Loader2 className="size-6 animate-spin text-[#52704D]" /></div> : isAdmin ? <AdminView tab={tab} setTab={setTab} context={context} tasks={tasks} knowledgeBases={knowledgeBases} taskForm={taskForm} setTaskForm={setTaskForm} knowledgeForm={knowledgeForm} setKnowledgeForm={setKnowledgeForm} memberText={memberText} setMemberText={setMemberText} busy={busy} createTask={createTask} createKnowledge={createKnowledge} importMembers={importMembers} /> : <LearnerView context={context} tasks={learnerTasks} inviteCode={inviteCode} setInviteCode={setInviteCode} busy={busy} join={join} updateContext={updateContext} openTask={openTask} />}
      </div>
    </main>
  )
}
...
function AdminView(props: {
  ...
  setTab: (value: "tasks" | "knowledge" | "members") => void
  ...
  const { tab, setTab, context, tasks, knowledgeBases, taskForm, setTaskForm, knowledgeForm, setKnowledgeForm, memberText, setMemberText, busy, createTask, createKnowledge, importMembers } = props
  return <>
    <nav className="mt-5 flex flex-wrap gap-2" aria-label="企业管理模块"><Link to="/enterprise/dashboard" className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#D7E1D4] bg-white px-4 text-xs font-bold text-[#61705F] transition hover:bg-[#F0F6EE]"><BarChart3 className="size-4" />运营看板</Link>{[["tasks", "任务发布", ClipboardList], ["knowledge", "岗位知识库", Library], ["members", "成员加入", UsersRound]] .map(([id, label, Icon]) => <button key={id as string} type="button" onClick={() => setTab(id as "tasks" | "knowledge" | "members")} className={cn("inline-flex h-10 items-center gap-2 rounded-xl border px-4 text-xs font-bold transition", tab === id ? "border-[#537E52] bg-[#52704D] text-white" : "border-[#D7E1D4] bg-white text-[#61705F] hover:bg-[#F0F6EE]")}><Icon className="size-4" />{label as string}</button>)}</nav>
    {tab === "tasks" && <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_390px]"><div className="rounded-2xl border border-[#DCE5D9] bg-white p-5"><SectionHeading icon={ClipboardList} title="已发布任务" detail="培训任务与普通阅读任务使用同一发布入口。" /><div className="mt-4 space-y-2.5">{tasks.length ? tasks.map((task) => <div key={task.id} className="rounded-xl border border-[#E6ECE3] bg-[#FBFDFB] p-3.5"><div className="flex items-start justify-between gap-3"><div><span className={cn("rounded-full px-2 py-1 text-[9px] font-bold", task.task_type === "training" ? "bg-[#E7EDF8] text-[#315E83]" : "bg-[#FFF1D9] text-[#946B27]")}>{task.task_type === "training" ? "岗位训练" : "普通阅读"}</span><h3 className="mt-2 text-sm font-bold text-[#293D2A]">{task.title}</h3></div><span className="rounded-full bg-[#EAF4E7] px-2 py-1 text-[9px] font-bold text-[#52704D]">{taskStatus[task.status]}</span></div><p className="mt-2 text-xs leading-5 text-[#738071]">{task.description || "暂无任务说明"}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px] text-[#7C8879]"><span>目标岗位：{task.target_role || "全员"}</span><span>·</span><span>{task.due_label}</span></div></div>) : <EmptyState text="还没有任务，右侧创建第一条演示任务。" />}</div></div><TaskComposer form={taskForm} setForm={setTaskForm} knowledgeBases={knowledgeBases} busy={busy} onSubmit={createTask} /></section>}
    {tab === "knowledge" && <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]"><div className="rounded-2xl border border-[#DCE5D9] bg-white p-5"><SectionHeading icon={Library} title="按岗位复用的企业知识库" detail="同一岗位的多个任务可以绑定同一资料集合。" /><div className="mt-4 grid gap-3 md:grid-cols-2">{knowledgeBases.length ? knowledgeBases.map((item) => <article key={item.id} className="rounded-xl border border-[#E3EBDD] bg-[#FBFDFB] p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-bold text-[#293D2A]">{item.name}</h3><p className="mt-1 text-[10px] font-semibold text-[#668064]">{item.target_role}</p></div><span className="rounded-full bg-[#EAF4E7] px-2 py-1 text-[9px] font-bold text-[#52704D]">{item.material_count} 份资料</span></div><p className="mt-3 text-xs leading-5 text-[#738071]">{item.description || "暂无说明"}</p><div className="mt-3 space-y-1">{item.materials.slice(0, 3).map((material, index) => <div key={`${item.id}-${index}`} className="flex gap-2 text-[10px] text-[#778477]"><Check className="mt-0.5 size-3 text-[#6F976B]" />{material.title}</div>)}</div></article>) : <EmptyState text="创建岗位知识库后，任务可以直接复用它。" />}</div></div><form onSubmit={createKnowledge} className="rounded-2xl border border-[#DCE5D9] bg-white p-5"><SectionHeading icon={FilePlus2} title="创建岗位资料库" detail="先建岗位范围，再逐条补充材料。" /><TextInput label="知识库名称" value={knowledgeForm.name} onChange={(value) => setKnowledgeForm({ ...knowledgeForm, name: value })} placeholder="例如：FDE 入职资料库" required /><TextInput label="目标岗位" value={knowledgeForm.target_role} onChange={(value) => setKnowledgeForm({ ...knowledgeForm, target_role: value })} placeholder="例如：前线部署工程师" required /><TextArea label="用途说明" value={knowledgeForm.description} onChange={(value) => setKnowledgeForm({ ...knowledgeForm, description: value })} placeholder="这套资料解决什么岗位问题？" /><TextArea label="资料条目（每行一条）" value={knowledgeForm.materials} onChange={(value) => setKnowledgeForm({ ...knowledgeForm, materials: value })} placeholder="岗位交付检查清单\n现场问题复盘模板" /><button disabled={busy === "knowledge-create"} className="mt-3 inline-flex h-10 items-center gap-2 rounded-xl bg-[#52704D] px-4 text-xs font-bold text-white disabled:opacity-50">{busy === "knowledge-create" ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />}创建知识库</button></form></section>}
...
function LearnerView({ context, tasks, inviteCode, setInviteCode, busy, join, updateContext, openTask }: { context: Context | null; tasks: Task[]; inviteCode: string; setInviteCode: (value: string) => void; busy: string; join: () => void; updateContext: (patch: Partial<Context>) => Promise<void>; openTask: (task: Task) => Promise<void> }) {
  const [role, setRole] = useState(context?.target_role || "")
  const hasEnterprise = Boolean(context?.enterprise)
  return <>
    {!hasEnterprise ? <section className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_390px]"><div className="rounded-2xl border border-[#DCE5D9] bg-white p-6"><span className="text-[10px] font-extrabold tracking-[.14em] text-[#5B7658]">加入企业</span><h2 className="mt-2 text-xl font-bold text-[#293D2A]">输入邀请码，接收岗位任务</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[#738071]">加入后只属于这个企业，企业管理员可以直接下发任务和查看训练状态。演示环境邀请码为 <strong className="text-[#52704D]">SM-DEMO</strong>。</p><div className="mt-5 flex max-w-md gap-2"><input value={inviteCode} onChange={(event) => setInviteCode(event.target.value)} className="h-11 min-w-0 flex-1 rounded-xl border border-[#D7E1D4] px-3 text-sm outline-none focus:border-[#52704D]" aria-label="企业邀请码" /><button type="button" onClick={join} disabled={busy === "join"} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#52704D] px-4 text-xs font-bold text-white disabled:opacity-50">{busy === "join" ? <Loader2 className="size-4 animate-spin" /> : <KeyRound className="size-4" />}加入企业</button></div></div><LearnerIdentity context={context} role={role} setRole={setRole} updateContext={updateContext} /></section> : <><section className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#DCE5D9] bg-white p-4"><div><span className="text-[10px] font-bold text-[#758372]">当前企业</span><h2 className="mt-1 text-base font-bold text-[#293D2A]">{context?.enterprise?.name}</h2></div><span className="rounded-full bg-[#EAF4E7] px-3 py-1.5 text-[10px] font-bold text-[#52704D]">成员身份 · {context?.enterprise?.member_role === "learner" ? "学习者" : context?.enterprise?.member_role}</span></section><div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><section className="rounded-2xl border border-[#DCE5D9] bg-white p-5"><SectionHeading icon={ClipboardList} title="我的企业任务" detail="培训任务进入岗位训练中心，阅读任务打开独立资料页。" /><div className="mt-4 space-y-3">{tasks.length ? tasks.map((task) => <article key={task.id} className="rounded-xl border border-[#E3EBDD] bg-[#FBFDFB] p-4"><div className="flex items-start justify-between gap-3"><div><span className={cn("rounded-full px-2 py-1 text-[9px] font-bold", task.task_type === "training" ? "bg-[#E7EDF8] text-[#315E83]" : "bg-[#FFF1D9] text-[#946B27]")}>{task.task_type === "training" ? "岗位训练任务" : "普通阅读任务"}</span><h3 className="mt-2 text-sm font-bold text-[#293D2A]">{task.title}</h3></div><span className="shrink-0 rounded-full bg-[#F0F4EE] px-2 py-1 text-[9px] font-bold text-[#63735F]">{taskStatus[task.assignment_status || "pending"]}</span></div><p className="mt-2 text-xs leading-5 text-[#738071]">{task.description}</p><div className="mt-3 flex items-center justify-between gap-3"><span className="text-[10px] text-[#7C8879]">{task.due_label} · {task.target_role || "全员"}</span><button type="button" onClick={() => void openTask(task)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#52704D] px-3 text-[10px] font-bold text-white">{task.assignment_status === "pending" ? "接受并打开" : task.task_type === "training" ? "进入岗位训练" : "打开资料"}<ArrowRight className="size-3.5" /></button></div></article>) : <EmptyState text="企业管理员还没有给你发布任务。" />}</div></section><LearnerIdentity context={context} role={role} setRole={setRole} updateContext={updateContext} /></div></>}
  </>
}

function LearnerIdentity({ context, role, setRole, updateContext }: { context: Context | null; role: string; setRole: (value: string) => void; updateContext: (patch: Partial<Context>) => Promise<void> }) {
  return <section className="rounded-2xl border border-[#DCE5D9] bg-white p-5"><SectionHeading icon={ShieldCheck} title="学习身份" detail="用于顶部身份展示和岗位任务匹配。" /><div className="mt-4 rounded-xl bg-[#F7FBF5] p-3 text-xs leading-5 text-[#687667]">{context?.name || "学习者"} · {context?.learner_type === "worker" ? context.company || "从业者" : context?.study_stage || "学生"}<br />当前岗位：{context?.target_role || "尚未补充"}</div><TextInput label="目标岗位" value={role} onChange={setRole} placeholder="例如：前线部署工程师" /><button type="button" onClick={() => void updateContext({ target_role: role })} className="mt-3 inline-flex h-9 items-center gap-2 rounded-lg border border-[#C8D8C4] bg-white px-3 text-[10px] font-bold text-[#52704D]"><Check className="size-3.5" />保存身份</button></section>
}
