...
const PUBLISHED_TASKS_PER_PAGE = 5
const LEARNER_TASKS_PER_PAGE = 4
...
function AdminView(props: {
  ...
  return <>
    {tab === "tasks" && <section className="mt-4 grid gap-4 xl:grid-cols-[390px_minmax(0,1.1fr)]"><TaskComposer form={taskForm} setForm={setTaskForm} knowledgeBases={knowledgeBases} busy={busy} onSubmit={createTask} /><PublishedTaskList tasks={tasks} /></section>}
...
function PublishedTaskList({ tasks }: { tasks: Task[] }) {
  const [page, setPage] = useState(1)
  const totalPages = Math.max(1, Math.ceil(tasks.length / PUBLISHED_TASKS_PER_PAGE))
  const currentPage = Math.min(page, totalPages)
  const visibleTasks = useMemo(() => {
    const start = (currentPage - 1) * PUBLISHED_TASKS_PER_PAGE
    return tasks.slice(start, start + PUBLISHED_TASKS_PER_PAGE)
  }, [currentPage, tasks])

  useEffect(() => { setPage(1) }, [tasks.length])

  return <div className="rounded-2xl border border-[#DCE5D9] bg-white p-5">
    <SectionHeading icon={ClipboardList} title="已发布任务" detail={`共 ${tasks.length} 个任务，每页显示 ${PUBLISHED_TASKS_PER_PAGE} 个。`} />
    <div className="mt-4 space-y-2.5">
      {visibleTasks.length ? visibleTasks.map((task) => <div key={task.id} className="rounded-xl border border-[#E6ECE3] bg-[#FBFDFB] p-3.5"><div className="flex items-start justify-between gap-3"><div><span className={cn("rounded-full px-2 py-1 text-[9px] font-bold", task.task_type === "training" ? "bg-[#E7EDF8] text-[#315E83]" : "bg-[#FFF1D9] text-[#946B27]")}>{task.task_type === "training" ? "岗位训练" : "普通阅读"}</span><h3 className="mt-2 text-sm font-bold text-[#293D2A]">{task.title}</h3></div><span className="rounded-full bg-[#EAF4E7] px-2 py-1 text-[9px] font-bold text-[#52704D]">{taskStatus[task.status]}</span></div><p className="mt-2 text-xs leading-5 text-[#738071]">{task.description || "暂无任务说明"}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px] text-[#7C8879]"><span>目标岗位：{task.target_role || "全员"}</span><span>·</span><span>{task.due_label}</span></div></div>) : <EmptyState text="还没有任务，请创建第一条企业任务。" />}
    </div>
    <TaskPager currentPage={currentPage} totalPages={totalPages} onChange={setPage} ariaLabel="已发布任务分页" />
  </div>
}
...
function LearnerView({ context, tasks, taskView, setTaskView, inviteCode, setInviteCode, busy, join, openTask }: { context: Context | null; tasks: Task[]; taskView: "pending" | "completed"; setTaskView: (value: "pending" | "completed") => void; inviteCode: string; setInviteCode: (value: string) => void; busy: string; join: () => void; openTask: (task: Task) => Promise<void> }) {
  ...
  const [page, setPage] = useState(1)
  const totalPages = Math.max(1, Math.ceil(visibleTasks.length / LEARNER_TASKS_PER_PAGE))
  const currentPage = Math.min(page, totalPages)
  const start = (currentPage - 1) * LEARNER_TASKS_PER_PAGE
  const paginatedTasks = visibleTasks.slice(start, start + LEARNER_TASKS_PER_PAGE)

  useEffect(() => { setPage(1) }, [taskView, visibleTasks.length])

  return <>
    {!hasEnterprise ? <section className="mt-5 rounded-2xl border border-[#DCE5D9] bg-white p-6"><span className="text-[10px] font-extrabold tracking-[.14em] text-[#5B7658]">加入企业</span><h2 className="mt-2 text-xl font-bold text-[#293D2A]">输入邀请码，接收岗位任务</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[#738071]">加入后只属于这个企业，企业管理员可以直接下发任务和查看训练状态。当前邀请码为 <strong className="text-[#52704D]">SM-DEMO</strong>。</p><div className="mt-5 flex max-w-md gap-2"><input value={inviteCode} onChange={(event) => setInviteCode(event.target.value)} className="h-11 min-w-0 flex-1 rounded-xl border border-[#D7E1D4] px-3 text-sm outline-none focus:border-[#52704D]" aria-label="企业邀请码" /><button type="button" onClick={join} disabled={busy === "join"} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#52704D] px-4 text-xs font-bold text-white disabled:opacity-50">{busy === "join" ? <Loader2 className="size-4 animate-spin" /> : <KeyRound className="size-4" />}加入企业</button></div></section> : <section className="mt-5 rounded-2xl border border-[#DCE5D9] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><SectionHeading icon={ClipboardList} title="我的企业任务" detail={`待完成和已完成任务均每页显示 ${LEARNER_TASKS_PER_PAGE} 个。`} /><div className="flex rounded-xl border border-[#D7E1D4] bg-[#F7F9F6] p-1"><button type="button" onClick={() => setTaskView("pending")} className={cn("h-9 rounded-lg px-3 text-[10px] font-bold transition", taskView === "pending" ? "bg-[#52704D] text-white shadow-sm" : "text-[#687667]")}>待完成 {pendingTasks.length}</button><button type="button" onClick={() => setTaskView("completed")} className={cn("h-9 rounded-lg px-3 text-[10px] font-bold transition", taskView === "completed" ? "bg-[#52704D] text-white shadow-sm" : "text-[#687667]")}>已完成 {completedTasks.length}</button></div></div><div className="mt-4 space-y-3">{paginatedTasks.length ? paginatedTasks.map((task) => <article key={task.id} className="rounded-xl border border-[#E3EBDD] bg-[#FBFDFB] p-4"><div className="flex items-start justify-between gap-3"><div><span className={cn("rounded-full px-2 py-1 text-[9px] font-bold", task.task_type === "training" ? "bg-[#E7EDF8] text-[#315E83]" : "bg-[#FFF1D9] text-[#946B27]")}>{task.task_type === "training" ? "岗位训练任务" : "普通阅读任务"}</span><h3 className="mt-2 text-sm font-bold text-[#293D2A]">{task.title}</h3></div><span className="shrink-0 rounded-full bg-[#F0F4EE] px-2 py-1 text-[9px] font-bold text-[#63735F]">{taskStatus[task.assignment_status || "pending"]}</span></div><p className="mt-2 text-xs leading-5 text-[#738071]">{task.description}</p><div className="mt-3 flex items-center justify-between gap-3"><span className="text-[10px] text-[#7C8879]">{task.due_label} · {task.target_role || "全员"}</span><button type="button" onClick={() => void openTask(task)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#52704D] px-3 text-[10px] font-bold text-white">{task.assignment_status === "pending" ? "接受并打开" : task.task_type === "training" ? "进入岗位训练" : "打开资料"}<ArrowRight className="size-3.5" /></button></div></article>) : <EmptyState text={taskView === "completed" ? "还没有已完成任务。" : "当前没有待完成任务。"} />}</div><TaskPager currentPage={currentPage} totalPages={totalPages} onChange={setPage} ariaLabel={`${taskView === "completed" ? "已完成" : "待完成"}任务分页`} /></section>}
  </>
}

function TaskPager({ currentPage, totalPages, onChange, ariaLabel }: { currentPage: number; totalPages: number; onChange: (page: number) => void; ariaLabel: string }) {
  if (totalPages <= 1) return null
  return <nav className="mt-5 flex flex-wrap items-center justify-center gap-1.5 border-t border-[#E6ECE3] pt-4" aria-label={ariaLabel}>
    <button type="button" onClick={() => onChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="grid size-8 place-items-center rounded-lg border border-[#D7E1D4] text-[#52704D] transition hover:bg-[#F1F6EF] disabled:cursor-not-allowed disabled:opacity-35" aria-label="上一页"><ChevronLeft className="size-4" /></button>
    {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} type="button" onClick={() => onChange(pageNumber)} className={cn("grid size-8 place-items-center rounded-lg border text-[10px] font-bold transition", currentPage === pageNumber ? "border-[#52704D] bg-[#52704D] text-white" : "border-[#D7E1D4] text-[#687667] hover:bg-[#F1F6EF]")} aria-label={`第 ${pageNumber} 页`} aria-current={currentPage === pageNumber ? "page" : undefined}>{pageNumber}</button>)}
    <button type="button" onClick={() => onChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="grid size-8 place-items-center rounded-lg border border-[#D7E1D4] text-[#52704D] transition hover:bg-[#F1F6EF] disabled:cursor-not-allowed disabled:opacity-35" aria-label="下一页"><ChevronRight className="size-4" /></button>
    <span className="ml-1 text-[10px] font-semibold text-[#7A8677]">第 {currentPage} / {totalPages} 页</span>
  </nav>
}
