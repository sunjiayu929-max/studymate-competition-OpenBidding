...
type Content = { courses: number; knowledge_chunks: number; enterprise_knowledge_bases: number; resources: number; training_runs: Record<string, number>; enterprise_tasks: number; updated_at: string }
  ...
  return (
    <main className="app-page paper-theme min-h-dvh bg-[#F3F0E7] px-3 pb-8 pt-3 sm:px-5 lg:px-7">
      <div className="mx-auto max-w-[1500px]">
        ...
        <nav className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-4" aria-label="系统管理视图">
          {tabs.map((tab) => <button key={tab.id} type="button" onClick={() => setView(tab.id)} className={cn("rounded-xl border px-3 py-3 text-left transition", view === tab.id ? "border-[#B85C3E] bg-[#9A4E35] text-white shadow-[0_8px_18px_rgba(154,78,53,.16)]" : "border-[#D7D1C4] bg-[#FFFEFA] text-[#68736F] hover:bg-white")}><span className="block text-xs font-bold">{tab.label}</span><span className={cn("mt-1 block text-[10px]", view === tab.id ? "text-[#F9E7DD]" : "text-[#9AA09B]")}>{tab.detail}</span></button>)}
        </nav>
        ...
        {loading && !overview ? <div className="grid min-h-[35vh] place-items-center"><RefreshCw className="size-6 animate-spin text-[#9A4E35]" /></div> : overview && <>
          ...
          {view === "enterprises" && <EnterpriseView items={overview.enterprises} />}
          {view === "users" && <UserView users={users} query={query} setQuery={setQuery} onSearch={() => void load()} />}
...
function OverviewView({ data }: { data: Overview }) {
  ...
  return <>
    ...
    <section className="mt-4 rounded-2xl border border-[#D7D1C4] bg-[#FFFEFA] p-5"><SectionHeading icon={Building2} title="企业运营速览" detail="系统管理员可以查看全局规模，任务发布仍由企业管理员完成。" /><div className="mt-4 grid gap-2 md:grid-cols-2">{data.enterprises.slice(0, 4).map((item) => <EnterpriseRow key={item.id} item={item} />)}</div></section>
  </>
}

function EnterpriseView({ items }: { items: Enterprise[] }) { return <section className="mt-4 rounded-2xl border border-[#D7D1C4] bg-[#FFFEFA] p-5"><SectionHeading icon={Building2} title="企业管理" detail={`平台当前接入 ${items.length} 家企业，数据与企业端保持一致。`} /><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-xs"><thead className="border-b border-[#E7E2D8] text-[10px] font-bold text-[#929994]"><tr><th className="px-3 py-2">企业</th><th className="px-3 py-2">管理员</th><th className="px-3 py-2">成员</th><th className="px-3 py-2">任务</th><th className="px-3 py-2">资料库</th><th className="px-3 py-2">完成率</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-b border-[#F0ECE4]"><td className="px-3 py-3"><strong className="text-[#334934]">{item.name}</strong><span className="mt-1 block text-[10px] text-[#9AA09B]">邀请码 {item.invite_code}</span></td><td className="px-3 py-3 text-[10px] text-[#748078]">{item.owner.name}</td><td className="px-3 py-3 font-bold text-[#52704D]">{item.member_count}</td><td className="px-3 py-3 text-[#748078]">{item.published_task_count} 已发布</td><td className="px-3 py-3 text-[#748078]">{item.knowledge_base_count}</td><td className="px-3 py-3"><span className="rounded-full bg-[#EAF4E7] px-2 py-1 text-[10px] font-bold text-[#52704D]">{item.completion_rate}%</span></td></tr>)}</tbody></table></div></section> }

function UserView({ users, query, setQuery, onSearch }: { users: AdminUser[]; query: string; setQuery: (value: string) => void; onSearch: () => void }) { return <section className="mt-4 rounded-2xl border border-[#D7D1C4] bg-[#FFFEFA] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><SectionHeading icon={UsersRound} title="用户与成员" detail="只返回账号、身份和企业归属等展示字段。" /><div className="flex h-9 w-full max-w-xs items-center gap-2 rounded-lg border border-[#D7D1C4] bg-white px-2.5"><Search className="size-3.5 text-[#9AA09B]" /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onSearch() }} className="min-w-0 flex-1 bg-transparent text-xs outline-none" placeholder="搜索姓名或邮箱" /><button type="button" onClick={onSearch} className="text-[10px] font-bold text-[#9A4E35]">查询</button></div></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="border-b border-[#E7E2D8] text-[10px] font-bold text-[#929994]"><tr><th className="px-3 py-2">用户</th><th className="px-3 py-2">系统角色</th><th className="px-3 py-2">身份</th><th className="px-3 py-2">企业归属</th><th className="px-3 py-2">目标岗位</th><th className="px-3 py-2">状态</th></tr></thead><tbody>{users.map((item) => <tr key={item.id} className="border-b border-[#F0ECE4]"><td className="px-3 py-3"><strong className="text-[#334934]">{item.name}</strong><span className="mt-1 block text-[10px] text-[#9AA09B]">{item.email}</span></td><td className="px-3 py-3"><RoleBadge role={item.role} /></td><td className="px-3 py-3 text-[10px] text-[#748078]">{item.role === "student" ? `学生 · ${item.study_stage || "未填写阶段"}` : item.role === "worker" ? `从业者 · ${item.company || "未填写公司"}` : "平台管理"}</td><td className="px-3 py-3 text-[10px] text-[#748078]">{item.enterprise?.name || "未加入企业"}{item.enterprise_job_title && <span className="mt-1 block text-[#9AA09B]">{item.enterprise_job_title}</span>}</td><td className="px-3 py-3 text-[10px] text-[#748078]">{item.target_role || "未选择"}</td><td className="px-3 py-3"><span className={cn("rounded-full px-2 py-1 text-[10px] font-bold", item.is_active ? "bg-[#EAF4E7] text-[#52704D]" : "bg-[#F4E8E2] text-[#9A4E35]")}>{item.is_active ? "正常" : "已停用"}</span></td></tr>)}</tbody></table>{!users.length && <div className="py-12 text-center text-xs text-[#8B938D]">没有匹配的用户</div>}</div></section> }

function ContentView({ data }: { data: Content | null }) { return <section className="mt-4 rounded-2xl border border-[#D7D1C4] bg-[#FFFEFA] p-5"><SectionHeading icon={Database} title="内容与运行" detail="系统管理员视角的内容规模和训练运行状态。" />{data ? <><div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4"><Stat label="系统课程" value={data.courses} /><Stat label="系统知识分片" value={data.knowledge_chunks} /><Stat label="企业资料库" value={data.enterprise_knowledge_bases} /><Stat label="企业任务" value={data.enterprise_tasks} /></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><StatusCard label="生成资源" value={data.resources} icon={FileCheck2} tone="blue" /><StatusCard label="训练运行记录" value={Object.values(data.training_runs).reduce((sum, item) => sum + item, 0)} icon={Activity} tone="green" /><StatusCard label="运行中" value={data.training_runs.running || 0} icon={RefreshCw} tone="gold" /><StatusCard label="已完成" value={data.training_runs.completed || 0} icon={CheckCircle2} tone="teal" /></div><p className="mt-4 text-[10px] text-[#9AA09B]">数据更新时间：{new Date(data.updated_at).toLocaleString("zh-CN")}</p></> : <div className="py-12 text-center text-xs text-[#8B938D]">正在加载内容状态</div>}</section> }
