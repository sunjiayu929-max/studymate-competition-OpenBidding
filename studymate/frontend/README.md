...
前端使用 React 19、TypeScript 6、Vite 8。登录后的业务页面统一使用可折叠左侧应用壳，承载学习宇宙首页、岗位空间、岗位训练中心、AI 助教、私有知识库、PPT 生成、笔记、测验、实时报告、可视讲解、学习资源与职业探索。

首页首屏恢复为独立深色的“学习宇宙 · 实时指挥舱”：顶部显示北京时间、数据同步和当前目标岗位，左侧区分个人实时数据与平台基础能力，中央以学习者核心和七颗可点击能力星球组织真实入口，右侧订阅 workspace store 展示 Agent 实时状态，底部使用真实笔记、测验、岗位能力画像评估和训练时间戳形成学习脉冲与近 7 日趋势。个人数据为空或单个接口失败时展示对应空态或降级，不用平台数据或测试 fixture 冒充个人成绩。进入今日学习后继续使用原有暖白工作台。
