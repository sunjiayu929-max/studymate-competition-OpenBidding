...
前端使用 React 19、TypeScript 6、Vite 8。登录后的业务页面统一使用可折叠左侧应用壳，承载浅色品牌首页、课程空间、学习资源工坊、AI 助教、私有知识库、PPT 生成、笔记、测验、实时报告、可视讲解、学习资源与职业探索。

首页首屏是浅色品牌首页：顶部提供课程、学习工坊、AI 助教、报告与指南快捷入口，中央用彩色几何装饰和真实学习数据预览组织主 CTA；鼠标移动会驱动聚光、装饰视差和产品卡片轻微 3D 倾斜。移动端提供折叠菜单，且遵循 `prefers-reduced-motion`。登录/注册页使用双栏冷白渐变布局，左侧彩色角色会随鼠标和输入状态产生轻量反馈，右侧保留邮箱登录、注册、验证码、错误提示和密码显示切换。原“学习宇宙 · 实时指挥舱”通过 `?legacy-home=1` 保留为回归入口；个人数据为空或单个接口失败时展示对应空态/降级，不用平台数据或测试 fixture 冒充个人成绩。
