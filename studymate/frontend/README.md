...
前端使用 React 19、TypeScript 6、Vite 8。登录后的业务页面统一使用可折叠左侧应用壳，承载浅色品牌首页、岗位空间、学习资源工坊、AI 助教、私有知识库、PPT 生成、笔记、测验、实时报告、可视讲解、学习资源与职业探索。

首页首屏是浅色品牌首页：顶部提供目标岗位、学习工坊、AI 助教、报告与指南快捷入口，中央用彩色几何装饰和真实训练数据预览组织主 CTA；鼠标移动会驱动聚光、装饰视差和产品卡片轻微 3D 倾斜。移动端提供折叠菜单，且遵循 `prefers-reduced-motion`。登录/注册页使用双栏冷白渐变布局，左侧彩色角色会随鼠标和输入状态产生轻量反馈，右侧保留邮箱登录、注册、验证码、错误提示和密码显示切换。原“学习宇宙 · 实时指挥舱”通过 `?legacy-home=1` 保留为回归入口；个人数据为空或单个接口失败时展示对应空态/降级，不用平台数据或测试 fixture 冒充个人成绩。

应用壳、页面标题和快捷入口使用统一的彩色图标底座；学习画像雷达图采用渐变填充、折线网格、均值徽标和优势维度标签，保留原有维度切换与评分数据。
...
| `/courses` | 领域与目标岗位空间 |
| `/profile` | 学习画像与就业技能 |
| `/workspace`、`/workspace/r/:agentId` | 学习资源工坊（8 个核心 Agent 闭环）与资源详情 |
| `/tutor`、`/tutor/voice` | 岗位助教与语音助教 |
| `/knowledge` | 用户私有知识库、资料进度与内部检索测试 |
| `/rag`、`/rag/source/:chunkId` | 岗位知识检索与原文上下文页，不占一级导航 |
| `/ppt` | PPT 大纲、预览编辑与可编辑 `.pptx` 导出 |
| `/notes` | 智能笔记与错题整理 |
| `/quiz`、`/quiz/:id` | 测验库、作答与回顾 |
| `/report` | 学习报告与岗位建议 |
| `/concept`、`/concept/library` | 可视讲解与动画库 |
| `/resources` | 按岗位能力点检索真实视频与外部公开课程资源 |
| `/career` | 职业匹配与能力差距探索 |
| `/feedback` | 反馈中心 |
| `/guide` | 新手指引与功能地图 |
| `/tests` | 管理员和评委测试管理 |
...
助教的 SSE 生成状态由模块级状态管理保持，可在页面或抽屉卸载后继续接收，并按用户与岗位知识边界隔离；底层仍使用兼容的 `course_id` 键。
...
PPT 大纲与单页重写调用 `/api/ppt/outline` 和 `/api/ppt/rewrite`，浏览器只发送 Qwen/DeepSeek/讯飞星火/MiMo 标识。未配置所选模型时页面明确提示，只有用户点击“明确使用本地策略”才走确定性降级，不会静默切换模型。PPTX 运行库保持按路由懒加载。
...
- `check-suggestion5.mjs` 覆盖人才呀岗位能力点匹配、B站真实视频直达、稳定搜索入口和报告文案。
- `check-suggestion6.mjs` 覆盖助教正文换行、长截图模式、右栏雷达填充和蛇形学习路径。
- `check:phase3` 静态核对数字人五状态媒体、响应式 WebP、语音用户手势策略、评委模式隔离恢复和落地页资源接线。
- `check:e2e` 自行启动已构建预览，以安全测试夹具覆盖登录、宇宙、岗位/RAG 来源、画像首屏、可视讲解时间轴、全局助教、语音降级，以及私库、PPT、模型选择、报告和管理员数据健康页；麦克风、全屏与全部后端/外部接口均被拦截，不写正式数据。
- `check:universe` 独立覆盖北京时间、平台/个人数据区分、8 个核心 Agents、七颗能力星球、主 CTA/岗位选择 CTA、真实空态与真实事件态、减弱动态、滚离暂停，并检查 1366×768、1440×900、1920×1080 无横向溢出。
